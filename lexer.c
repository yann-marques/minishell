...
int	replace_var_call(t_env *env, char **str, int i)
{
	char	*var;
	char	*begin;
	char	*end;
	...
	if (*str[i] != '$')
		return (1);
	j = ft_strlen_to(&*str[i + 1], ' ');
	if (j > ft_strlen(&*str[i + 1], '"'))
		j = ft_strlen_to(&*str[i + 1], '"');
	begin = ft_strndup(&*str[i + 1], j);
	if (!begin)
		return (0);
	var = get_var_value(env, begin);
	free(begin);
	begin = ft_strndup(*str, i);
	if (!begin)
		return (0);
	end = ft_strndup(&*str[i + j], 0);
	if (!end)
		return (free(begin));
	// var = ft_join_three(begin, var, end);
	// if ()
	return (1);
}

void	var_to_value(t_ms *head)
{
	t_token	*tmp;
	int		i;
	int		k;
